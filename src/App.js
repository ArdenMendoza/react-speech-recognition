...
import './App.css';
import loader from './loaderSvg.svg';
...
function App() {
  ...
  const convoComponent = (convo) => <div>{convo.map(f => { return <div key={timeStamp}>{f}</div> })}</div>;
  return (
    <div className="App" style={{ height: '100%', overflow: 'none' }}>
      <div style={{
        ...
        <div style={{ position: 'absolute', left: 'calc(50% - 89px)', top: 'calc(50% - 53px)' }}>
          <img src={'https://nerdylifeofmine.files.wordpress.com/2017/11/loading.gif'} alt="Loader" style={{ height: '100px', display: isListening ? 'none' : 'block' }} />
        </div>
        <div style={{ height: '50%', width: '50%', margin: '200px auto', overflow: 'auto'}}>
